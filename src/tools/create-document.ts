...
const createDocumentSchema = z.object({
  ...
  document_kind: z
    ...
    .default('formal')
    .describe('Formal documents must be registered in Documentation Hub; temporary is only for disposable tests.'),
  documentation: documentationSchema.optional().describe('Documentation Hub metadata for a formal document.'),
});
...
export function registerCreateDocumentTool(server: McpServer, services: Services): void {
  server.registerTool(
    ...
    {
      description:
        'Create a Feishu cloud document, apply sharing, read it back, and automatically register every formal document in the unique AI Workspace Documentation Hub. Defaults to formal and company_editable; temporary is only for disposable validation documents.',
  ...
  const value = {
    ...
    next_action:
      stage === 'documentation_hub_registration'
        ...
        : 'Do not retry create_document. Read back this existing document, then complete Documentation Hub registration.',
  };
  return {
